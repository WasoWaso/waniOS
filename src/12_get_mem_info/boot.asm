...
.10E:                                                   ; }
        ...
        ;************************************************
        mov     ax , 3939                               ;
        cdecl itoa, ax, .s0, 8, 10, 0b0001              ; itoa(AX)
...
;(関数の概要())
; 0. メモリ情報の取得と表示を行います、同時にそれらを担当する関数を作成したので実験しています.
; 1. '.10Q' ACPI情報のアドレスを表示します、get_mem_info関数ではACPI情報を得ると0x7E04へ
;    値を保存するので、その値(ACPI_DATA.adr)をCMPにかけ文字列を表示するか否かを決定します.
